...
# HTML served as string (LaTeX parts removed)
html_template = '''
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        ...
        <form id="integralForm">
            <label for="function">Function (example: x*2 + y*2 or sin,cos,log, e):</label>
            <input type="text" id="function" placeholder="Enter function..." required />
            <label for="lower_limit_x">Lower Limit (outer variable ):</label>
            <input type="text" id="lower_limit_x" value="0" />
            <label for="upper_limit_x">Upper Limit (outer variable ):</label>
            <input type="text" id="upper_limit_x" value="1" />
            <label for="lower_limit_y">Lower Limit (inner variable ):</label>
            <input type="text" id="lower_limit_y" value="0" />
            <label for="upper_limit_y">Upper Limit (inner variable ):</label>
        ...
        <div class="result-box">
            <h2>🎉 Result:</h2>
            <pre id="numerical-result"></pre>
        </div>

        <div class="result-box">
            <h2>🔢 Fraction Value:</h2>
            <pre id="fraction-result"></pre>
        </div>
    </div>
    ...
    <script>
        document.getElementById('integralForm').onsubmit = async function (event) {
            ...
            document.getElementById('numerical-result').innerText = result.result || result.error;
            document.getElementById('fraction-result').innerText = result.fraction_result || ''; // Initialize as empty

            if (result.result || result.fraction_result) {
                confetti({
                    particleCount: 150,
                    spread: 70,
                    origin: { y: 0.6 }
...
def double_integral():
    ...
    try:
        replacements = {'e': 'sp.E', 'pi': 'sp.pi'}
        for key, value in replacements.items():
            function_expr = function_expr.replace(key, value)

        lower_limit_x = sp.sympify(lower_limit_x)
        upper_limit_x = sp.sympify(upper_limit_x)
        lower_limit_y = sp.sympify(lower_limit_y)
        upper_limit_y = sp.sympify(upper_limit_y)
        ...
        evaluated_result_float = integral_result.evalf()

        try:
            fractional_result = sp.Rational(integral_result).limit_denominator()
            fraction_str = str(fractional_result)
        except (TypeError, ValueError):
            fraction_str = "Could not represent as fraction"

        return jsonify({
            "result": str(evaluated_result_float),
